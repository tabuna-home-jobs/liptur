                        ...
                        <tr>
                            <td style="padding: 40px; text-align: left; font-family: sans-serif; font-size: 15px; line-height: 20px; color: #555555;">
                                ...
                                <p style="font-size: 16px; text-align: left;">
                                    Здравствуйте!  <br><br>
                                    На сайте Liptur.ru пользователь подал заявку на бронирование <br>
                                    Имя пользователя: {{ $param["user"]->name }}<br>
                                    Email пользователя: {{ $param["user"]->email }}<br>
                                    Телефон пользователя: {{ $param["user"]->phone}}<br>
                                    Дата заявки: {{$param["reservation"]->date}}<br>
                                    Количество человек: {{$param["reservation"]->quantity}}<br><br>

                                    Страница на которой подана заявка:<br>
                                    название: {{$param["post"]->getContent("name")}}<br>
                                    @if ($param["post_url"])ссылка на страницу: <a href="{{$param["post_url"]}}">{{$param["post_url"]}}</a><br>@endif
                                    Контактная информация, отправленная пользователю:<br>
                                    Email: {{$param["post_email"]}}<br>
                                    @if ($param["post"]->getContent("phone"))Телефон: {{$param["post"]->getContent("phone")}}<br>@endif
                                </p>
                                <br>
                            </td>
                        </tr>
