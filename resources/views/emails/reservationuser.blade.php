                    ...
                    <table role="presentation" aria-hidden="true" cellspacing="0" cellpadding="0" border="0"
                        ...
                        <tr>
                            <td style="padding: 40px; text-align: left; font-family: sans-serif; font-size: 15px; line-height: 20px; color: #555555;">
                                ...
                                <p style="font-size: 16px; text-align: center">Вы выполнили бронирование на сайте liptur.ru
                                </p>
                                <p style="font-size: 16px; text-align: left;">
                                    Здравствуйте {{ $param["user"]->name }} ! <br><br>
                                    Спасибо за Вашу заявку на бронирование на сайте   Liptur.ru<br><br>


                                    Вы подали заявку на бронирование мест на "{{$param["post"]->getContent("name")}}"<br>
                                    email: {{$param["post_email"]}}<br>
                                    @if ($param["post"]->getContent("phone")) Контактный телефон:{{$param["post"]->getContent("phone")}}<br>@endif
                                    @if ($param["post_url"])адрес страницы: <a href="{{$param["post_url"]}}">{{$param["post_url"]}}</a>@endif
                                    <br><br>
                                    Дата бронирования:{{$param["reservation"]->date}}<br>
                                    Количество человек:{{$param["reservation"]->quantity}}
                                </p>
                            </td>
                        </tr>
                    </table>
